...
@php
    ...
    $anuladosPct = $total > 0 ? round($anulados / $total * 100) : 0;
    $asignadosPct= $total > 0 ? round($asignados / $total * 100) : 0;

    $statuses = [
        ['label' => 'Asignados','val' => $asignados, 'pct' => $asignadosPct, 'bar' => 'bg-purple-500', 'text' => 'text-purple-600', 'badge' => 'bg-purple-100 text-purple-700'],
        ['label' => 'Limpios',  'val' => $limpios,  'pct' => $limpiosPct,  'bar' => 'bg-blue-500',   'text' => 'text-blue-600',   'badge' => 'bg-blue-100 text-blue-700'],
        ['label' => 'Llenos',   'val' => $llenos,   'pct' => $llenosPct,   'bar' => 'bg-yellow-400', 'text' => 'text-yellow-600', 'badge' => 'bg-yellow-100 text-yellow-700'],
        ['label' => 'Ventas',   'val' => $ventas,   'pct' => $ventasPct,   'bar' => 'bg-green-500',  'text' => 'text-green-600',  'badge' => 'bg-green-100 text-green-700'],
        ['label' => 'Anulados', 'val' => $anulados, 'pct' => $anuladosPct, 'bar' => 'bg-red-500',    'text' => 'text-red-500',    'badge' => 'bg-red-100 text-red-700'],
    ];
@endphp
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-0.5">
                Hoy es {{ now()->translatedFormat('l, d \d\e F \d\e Y') }}
            </p>
        </div>
        <a href="{{ route('contracts.index') }}"
           class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white text-sm font-semibold hover:opacity-90 transition-opacity shadow-sm">
    ...
    <section class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
        ...
        <div class="bg-white p-6 rounded-xl border border-purple-200 shadow-sm">
            ...
            <div class="mt-3 w-full bg-slate-100 h-1.5 rounded-full">
                <div class="bg-purple-500 h-full rounded-full" style="width: {{ max($asignadosPct, $asignados > 0 ? 4 : 0) }}%"></div>
            </div>
            <p class="text-xs text-slate-400 mt-1.5">{{ $asignadosPct }}% del total</p>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            ...
            <p class="text-3xl font-black text-slate-800">{{ number_format($limpios) }}</p>
            <div class="mt-3 w-full bg-slate-100 h-1.5 rounded-full">
                <div class="bg-blue-500 h-full rounded-full" style="width: {{ max($limpiosPct, $limpios > 0 ? 4 : 0) }}%"></div>
            </div>
            <p class="text-xs text-slate-400 mt-1.5">{{ $limpiosPct }}% del total</p>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div class="mt-3 w-full bg-slate-100 h-1.5 rounded-full">
                <div class="bg-yellow-400 h-full rounded-full" style="width: {{ max($llenosPct, $llenos > 0 ? 4 : 0) }}%"></div>
            </div>
            <p class="text-xs text-slate-400 mt-1.5">{{ $llenosPct }}% del total</p>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div class="mt-3 w-full bg-slate-100 h-1.5 rounded-full">
                <div class="bg-green-500 h-full rounded-full" style="width: {{ max($ventasPct, $ventas > 0 ? 4 : 0) }}%"></div>
            </div>
            <p class="text-xs text-slate-400 mt-1.5">{{ $ventasPct }}% del total</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <section class="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            ...
            <div class="p-5 border-b border-slate-100 flex items-center justify-between">
                <h3 class="text-base font-bold text-slate-800">Contratos recientes</h3>
                <a href="{{ route('contracts.index') }}"
                            ...
                            <tr class="hover:bg-slate-50/70 transition-colors">
                                ...
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-slate-700">{{ $contract->advisor->name ?? '—' }}</span>
                                    </div>
                                </td>
                                <td class="px-5 py-3.5">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
                <h3 class="text-base font-bold text-slate-800 mb-5">Estados de contratos</h3>
                    ...
                    @foreach ($statuses as $s)
                        <div>
                            ...
                            <div class="w-full bg-slate-100 rounded-full h-1.5">
                                <div class="{{ $s['bar'] }} h-1.5 rounded-full transition-all"
